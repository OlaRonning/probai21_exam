...
if __name__ == '__main__':
    ...
    if args.method == 0:
        ...
        method = MultiplicativeNormalizingFlow(data.xtr.shape[-1], 50, 1, batch_size=1000, num_epochs=1)
    elif args.method == 4:
    ...
    print(f'Method:{str(method)}, dataset:{args.dataset}, '
          f'RMSE:{rmse(data.yte, method.transform(data.xte)):.2f}, '
          f'PICP:{picp(data.yte, method.transform(data.xte, 100)):.2f}, '
          f'MPIW:{mpiw(method.transform(data.xte, 100))}')
