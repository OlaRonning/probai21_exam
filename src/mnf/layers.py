...
class MNFLayer(nn.Module):
    ...
    def log_prob_aux_back(self, aux_back):
        ...
        variance = torch.permute(self.variance, (-1, -2)).reshape((-1,
                                                                   self.in_channels))
        out_channels = torch.numel(self.mean.shape[:-1])
    ...
    def kl_divergence(self):
        ...
        aux = aux.reshape((1, -1, 1))

        kl_qw_pw = variance + aux.reshape((1, -1, 1)) * mean + torch.log(variance)
...
class MNFLinear(MNFLayer):
    def __init__(self, in_channels, out_channels, forward_bijections, backward_bijections):
        super(MNFLinear, self).__init__(in_channels, out_channels, forward_bijections, backward_bijections)

        self.mean = nn.Parameter(torch.empty((in_channels, out_channels), requires_grad=True))
        nn.init.xavier_normal_(self.mean)
        self.log_variance = nn.Parameter(
            torch.distributions.Normal(torch.exp(torch.tensor(-9)), 1e-3).sample((in_channels, out_channels)))
...
class MNFConv2D(MNFLayer):
    def __init__(self, in_channels, out_channels, kernel_size: Tuple, forward_bijections, backward_bijections,
                 stride=1):
        super(MNFConv2D, self).__init__(in_channels, out_channels, forward_bijections, backward_bijections)

        self.mean = nn.Parameter(torch.empty((*kernel_size, in_channels, out_channels), requires_grad=True))
        nn.init.xavier_normal_(self.mean)
        self.log_variance = nn.Parameter(
